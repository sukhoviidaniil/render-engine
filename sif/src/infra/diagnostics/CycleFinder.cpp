...
#include "sif/infra/diagnostics/CycleFinder.h"
...
namespace
{
    ...
    using VertexId = DirectedGraph<std::string>::VertexId;
    ...
    void strongConnect(VertexId v, const DirectedGraph<std::string>& graph, TarjanContext& ctx)
    {
...
namespace sif::infra::diag {
    std::vector<std::vector<VertexId>> findCycles(const DirectedGraph<std::string>& graph)
    {
        TarjanContext ctx;
